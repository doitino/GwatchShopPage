...
@WebServlet(name = "CT_Product_Nam", urlPatterns = "/CT_Product_Nam")
public class CT_Product_Nam extends HttpServlet {
...
//            if(count_after!=null) n= Integer.parseInt(count_after);
            int size = 12;
            int index = size *(n-1) ;
            int count = pr.countP("nam");
            int endpage =0;
            endpage= count /size;
            if(count%size != 0){
                endpage++;
            }
            List<Product> value = pr.phanTrangP(index , size,"nam");
